...
class HandoffSpecTests(unittest.TestCase):
    ...
    def mutated(self, name, change, *, reseal=False):
    ...
    def test_durable_process_exit_cannot_expect_unknown_commit(self):
        def change(b):
            case = next(c for c in b["cases"] if c["case_id"] == "E0-030")
            case.update(expected_check="unknown", expected_commit="unknown", expected_generation_delta=None)
        self.assertIn("E0_EXPECTATION", [i["code"] for i in self.mutated("cases.json", change)])

    def test_each_feature_combination_has_all_three_scenarios(self):
        def change(b):
            b["cases"] = [c for c in b["cases"] if c["stimulus"]["kind"] != "feature_combination"]
        self.assertIn("E0_COVERAGE", [i["code"] for i in self.mutated("cases.json", change)])

    def test_old_package_migration_requires_all_six_paths(self):
        def change(b):
            b["cases"] = [c for c in b["cases"] if c["stimulus"]["kind"] != "legacy_migration"]
        self.assertIn("E0_COVERAGE", [i["code"] for i in self.mutated("cases.json", change)])

    def test_flush_overflow_cannot_borrow_the_event_log_budget(self):
        def change(b):
            c = next(c for c in b["cases"] if c["case_id"] == "E0-017")
            c["stimulus"] = {"kind": "mandatory_bytes", "value": 8388609}
        self.assertIn("E0_BUDGET", [i["code"] for i in self.mutated("cases.json", change)])

    def test_contract_version_cannot_be_empty_independent_of_manifest(self):
        def change(b):
            b["record"]["contract_version"] = ""
            b["requests"]["prepare_handoff"]["record"]["contract_version"] = ""
        self.assertIn("E0_SCHEMA", [i["code"] for i in self.mutated("ready-state.json", change)])

    def test_contract_version_cannot_contain_only_whitespace(self):
        def change(b):
            b["record"]["contract_version"] = " \t "
            b["requests"]["prepare_handoff"]["record"]["contract_version"] = " \t "
        self.assertIn("E0_SCHEMA", [i["code"] for i in self.mutated("ready-state.json", change)])

    def test_each_single_matrix_row_is_required_not_only_group_labels(self):
        bundle = json.loads((BUNDLE / "cases.json").read_text(encoding="utf-8"))
        ids = [c["case_id"] for c in bundle["cases"] if c["stimulus"]["kind"] in {"feature_combination", "legacy_migration", "log_read_bytes"}]
        self.assertEqual(len(ids), 33)
        for case_id in ids:
            with self.subTest(case_id=case_id):
                def change(b):
                    b["cases"] = [c for c in b["cases"] if c["case_id"] != case_id]
                self.assertIn("E0_COVERAGE", [i["code"] for i in self.mutated("cases.json", change, reseal=True)])

    def test_legacy_number_and_nonblank_string_versions_still_work(self):
        for version in (1, 1.5, "1", "release-1"):
            with self.subTest(version=version):
                def change(b):
                    b["record"]["contract_version"] = version
                    b["requests"]["prepare_handoff"]["record"]["contract_version"] = version
                    digest = hashlib.sha256(json.dumps(b["record"], sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
                    for event in b["events"]:
                        event["record_sha256"] = digest
                self.assertEqual(self.mutated("ready-state.json", change, reseal=True), [])

    def test_resealing_does_not_hide_the_four_semantic_regressions(self):
        def r1(b):
            next(c for c in b["cases"] if c["case_id"] == "E0-030").update(expected_check="unknown", expected_commit="unknown", expected_generation_delta=None)
        def r2(b):
            b["cases"] = [c for c in b["cases"] if c["case_id"] != "E0-148"]
        def r3(b):
            next(c for c in b["cases"] if c["case_id"] == "E0-017")["stimulus"]["value"] = 8388609
        def r4(b):
            b["record"]["contract_version"] = ""
            b["requests"]["prepare_handoff"]["record"]["contract_version"] = ""
        for name, change, code in [("cases.json", r1, "E0_EXPECTATION"), ("cases.json", r2, "E0_COVERAGE"), ("cases.json", r3, "E0_BUDGET"), ("ready-state.json", r4, "E0_SCHEMA")]:
            with self.subTest(regression=change.__name__):
                codes = [i["code"] for i in self.mutated(name, change, reseal=True)]
                self.assertIn(code, codes)
                self.assertNotIn("E0_HASH", codes)
    ...
    def test_duplicate_json_keys_are_not_last_writer_wins(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            needle = '"fixture_version": ' + str(json.loads(text)["fixture_version"])
            self.assertIn(needle, text)
            (root / "cases.json").write_text(text.replace(needle, needle + ", " + needle, 1), encoding="utf-8")
            issues = validate_bundle(root)
        self.assertIn("E0_READ", [issue["code"] for issue in issues])
